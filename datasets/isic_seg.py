...
class ISICSegDataset(BaseImageDataset):
    def __init__(self, *argv, **kargs):
        ...
        self.labels = np.array([os.path.join(self.root, "labels", f"{file}_segmentation.png") for file in self.sample_list])[
            : int(len(self.sample_list) * self.size_rate)
        ]
        self.embeddings = np.array([os.path.join(self.root, "embeddings", f"{file}.npy")
                                     for file in self.sample_list])[: int(len(self.sample_list) * self.size_rate)]
    ...
    def _fetch_data(self, idx):
        ...
        if os.path.exists(emb_path):
            embedding = np.load(emb_path)  # shape: [2, 512]
            joint_embedding = embedding.astype(np.float32)
        else:
            print("emb_path:",emb_path)

            pass

        return {"image": image, "label": label, "embedding": joint_embedding}
...
class ISICGazeDataset(ISICSegDataset):
